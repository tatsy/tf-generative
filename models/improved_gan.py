...
def minibatch_discrimination(x, kernels=50, dims=5):
    with tf.name_scope('MinibatchDiscrimination'):
        size = x.get_shape()[1]
        W = tf.get_variable(shape=(size, kernels * dims), trainable=True, name='kernel')
        Ms = tf.tensordot(x, W, axes=1)
        x_i = tf.reshape(Ms, [-1, kernels, 1, dims])
        x_j = tf.reshape(Ms, [-1, 1, kernels, dims])
        x_i = tf.tile(x_i, [1, 1, kernels, 1])
        x_j = tf.tile(x_j, [1, kernels, 1, 1])
        norm = tf.reduce_sum(tf.abs(x_i - x_j), axis=3)
        Os = tf.reduce_sum(tf.exp(-norm), axis=2)
        return Os
    ...
    def __call__(self, inputs, training=True):
        with tf.variable_scope('discriminator', reuse=self.reuse):
            ...
            with tf.variable_scope('minibatch_discrimination'):
                x = minibatch_discrimination(x, kernels=50, dims=5)
                f = tf.identity(x)
...
class ImprovedGAN(BaseModel):
    ...
    def build_model(self):
        ...
        y_real, f_real = self.f_dis(self.x_train)

        E_f_fake = tf.reduce_mean(f_fake, axis=0)
        E_f_real = tf.reduce_mean(f_real, axis=0)
        self.gen_loss = tf.reduce_sum(tf.square(E_f_real - E_f_fake))
